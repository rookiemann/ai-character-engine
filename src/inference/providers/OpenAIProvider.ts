...
export class OpenAIProvider extends BaseProvider {
  private log = getLogger('openai-provider');
  ...
  async complete(request: InferenceRequest): Promise<InferenceResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new InferenceError(`OpenAI returned ${response.status}: ${errorText}`, 'openai');
      }
      ...
      if (!choice) {
        throw new InferenceError('No choices in OpenAI response', 'openai');
      }
      ...
      throw new InferenceError(`OpenAI request failed: ${message}`, 'openai');
    } finally {
      this.activeRequests--;
    }
  }
